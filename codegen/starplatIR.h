...
public:
  ...
  virtual void visitDeclarationStmt(const DeclarationStatement *dclstmt,
                                    mlir::SymbolTable *symbolTable) override {}

  virtual void visitTemplateDeclarationStmt(
      const TemplateDeclarationStatement *templateDeclStmt,
      mlir::SymbolTable *symbolTable) override {

    TemplateType *Type =
        static_cast<TemplateType *>(templateDeclStmt->gettype());
    ...
    if (std::string(Type->getGraphPropNode()->getPropertyType()) ==
        "propNode") {

      mlir::Type type;

      if (std::string(Type->getType()->getType()) == "int")
        type = builder.getType<mlir::starplat::PropNodeType>(
            builder.getI64Type(), graphId);
      else if (std::string(Type->getType()->getType()) == "bool")
        type = builder.getType<mlir::starplat::PropNodeType>(
            builder.getI1Type(), graphId);
      else {
  ...
  virtual void visitForallStmt(const ForallStatement *forAllStmt,
                               mlir::SymbolTable *symbolTable) override {
    ...
    if (expr->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
      ...
      const Methodcall *outermethodcall = static_cast<const Methodcall *>(
          memberaccess->getMethodCall()); // Filter Methodcall.
      ...
      if (memberaccess->getMemberAccessNode()) {
        ...
        const Memberaccess *nestedMemberaccess =
            static_cast<const Memberaccess *>(
                memberaccess->getMemberAccessNode());
        const Identifier *identifier = nestedMemberaccess->getIdentifier(); // g
        const Methodcall *innerMethodcall = static_cast<const Methodcall *>(
            nestedMemberaccess->getMethodCall());
        ...
        const Identifier *innerMethodcallIdentifier =
            innerMethodcall->getIdentifier();

        if (strcmp(innerMethodcallIdentifier->getname(), "nodes") == 0) {
          loopVarType = mlir::starplat::NodeType::get(builder.getContext());
          mlir::Value graph = NULL;
          loopVarOp = mlir::starplat::DeclareOp::create(
              builder, builder.getUnknownLoc(), loopVarType,
          ...
          llvm::outs() << "Error: Methodcall '"
                       << innerMethodcallIdentifier->getname()
                       << "' not Implemented.\n";
          return;
        }
      }
        ...
        if (methodcallin->getIsBuiltin()) {
          if (strcmp(methodcallin->getIdentifier()->getname(), "neighbors") ==
              0) {
            const Expression *idExpr =
                static_cast<const Expression *>(methodcallin->getParamLists());

            if (idExpr->getKind() == ExpressionKind::KIND_IDENTIFIER) {
              const Identifier *idParam =
                  static_cast<const Identifier *>(idExpr->getExpression());
          ...
          llvm::outs() << "Undefined method call: "
                       << methodcallin->getIdentifier()->getname();
          return;
        }
      }
      ...
      if (outermethodcall->getIsBuiltin()) {
        ...
        const Identifier *identifier1 =
            static_cast<const Identifier *>(outermethodcall->getIdentifier());
        if (strcmp(identifier1->getname(), "filter") == 0) {

          // loopAttr.push_back(builder.getStringAttr("filter"));
          filter = builder.getBoolAttr(1);

          const Expression *outer =
              static_cast<const Expression *>(outermethodcall->getParamLists());
          const BoolExpr *outerBoolExpr =
              static_cast<const BoolExpr *>(outer->getExpression());

          const Expression *lhsExpr =
              static_cast<const Expression *>(outerBoolExpr->getExpr1());
          const Expression *rhsExpr =
              static_cast<const Expression *>(outerBoolExpr->getExpr2());
          ...
          if (lhsExpr->getKind() == ExpressionKind::KIND_IDENTIFIER &&
              rhsExpr->getKind() == ExpressionKind::KIND_KEYWORD) {
            const Identifier *lhsIdentifier =
                static_cast<const Identifier *>(lhsExpr->getExpression());
            const Keyword *rhsKeyword =
            ...
            if (!lhsidSymbol) {
              llvm::outs() << "Error DXB: Identifier '"
                           << lhsIdentifier->getname() << "' not declared.\n";
              return;
            }
    ...
    auto loopOp = mlir::starplat::ForAllOp::create(
        builder, builder.getUnknownLoc(), loopOperands, loopAttrArray, filter,
        loopa);
  ...
  virtual void visitMemberaccessStmt(const MemberacceessStmt *MemberacceessStmt,
                                     mlir::SymbolTable *symbolTable) override {
    ...
    if (methodcallnode && methodcallnode->getIsBuiltin()) {
      if (std::strcmp(methodcallnode->getIdentifier()->getname(),
        ...
        for (Param *param : paramListVecvtor) {
          ...
          const auto *paramAssignment =
              static_cast<const ParameterAssignment *>(
          ...
          auto idSymbol = globalLookupOp(
              identifier->getname()); // globalLookupOp(identifier->getname());
          auto kwSymbol = globalLookupOp(
              keyword->getKeyword()); // globalLookupOp(keyword->getKeyword());

          if (!kwSymbol)
            llvm::outs() << "Error: Keyword '" << keyword->getKeyword()
                         << "' not declared.\n";
  ...
  virtual void visitIfStmt(const IfStatement *ifStmt,
                           mlir::SymbolTable *symbolTable) override {}

  virtual void visitBoolExpr(const BoolExpr *boolExpr,
                             mlir::SymbolTable *symbolTable) override {}

  virtual void visitIncandassignstmt(const Incandassignstmt *incandassignstmt,
                                     mlir::SymbolTable *symbolTable) override {}
  ...
  void visitAssignmentStmt(const AssignmentStmt *assignemntStmt,
                           mlir::SymbolTable *symbolTable) override {
  ...
  virtual void visitIdentifier(const Identifier *identifier,
                               mlir::SymbolTable *symbolTable) override {}
  ...
  virtual void
  visitParameterAssignment(const ParameterAssignment *paramAssignment,
                           mlir::SymbolTable *symbolTable) override {
    Identifier *identifier =
    ...
    if (globalLookupOp(identifier->getname())) {
      ...
      llvm::outs() << "error: " << identifier->getname()
                   << " not declared -_-\n";
      exit(1);
    }
  }
  ...
  virtual void visitTupleAssignment(const TupleAssignment *tupleAssignment,
    ...
    if (lhsexpr1->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
      const Memberaccess *lhs1MemberAccess =
          static_cast<const Memberaccess *>(lhsexpr1->getExpression());
      if (lhs1MemberAccess->getIdentifier() &&
          lhs1MemberAccess->getIdentifier2()) {
        if (globalLookupOp(lhs1MemberAccess->getIdentifier2()->getname())) {
          if (globalLookupOp(lhs1MemberAccess->getIdentifier()->getname())) {
            mlir::Value propOp =
                globalLookupOp(lhs1MemberAccess->getIdentifier2()->getname());
            mlir::Value varOp =
                globalLookupOp(lhs1MemberAccess->getIdentifier()->getname());
            gOperand1 = mlir::starplat::GetNodePropertyOp::create(
                builder, builder.getUnknownLoc(), builder.getI32Type(), varOp,
                propOp,
                builder.getStringAttr(
                    lhs1MemberAccess->getIdentifier2()->getname()));
          } else {
            llvm::outs() << "Error: Undefined Var `"
                         << lhs1MemberAccess->getIdentifier()->getname()
                         << "'/n.";
            return;
          }
        } else {
          llvm::outs() << "Error: Undefined Property `"
                       << lhs1MemberAccess->getIdentifier2()->getname()
                       << "'/n.";
          return;
        }
    ...
    if (lhsexpr2->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
      const Memberaccess *lhs2MemberAccess =
          static_cast<const Memberaccess *>(lhsexpr2->getExpression());
      if (lhs2MemberAccess->getIdentifier() &&
          lhs2MemberAccess->getIdentifier2()) {
        if (globalLookupOp(lhs2MemberAccess->getIdentifier2()->getname())) {
          if (globalLookupOp(lhs2MemberAccess->getIdentifier()->getname())) {
            mlir::Value propOp =
                globalLookupOp(lhs2MemberAccess->getIdentifier2()->getname());
            mlir::Value varOp =
                globalLookupOp(lhs2MemberAccess->getIdentifier()->getname());
            gOperand1 = mlir::starplat::GetNodePropertyOp::create(
                builder, builder.getUnknownLoc(), builder.getI32Type(), varOp,
                propOp,
                builder.getStringAttr(
                    lhs2MemberAccess->getIdentifier2()->getname()));
          } else {
            llvm::outs() << "Error: Undefined Var `"
                         << lhs2MemberAccess->getIdentifier()->getname()
                         << "'/n.";
            return;
          }
        } else {
    ...
    if (rhsexpr1->getKind() == ExpressionKind::KIND_METHODCALL) {
      ...
      if (rhsMethodCall->getIsBuiltin() &&
        ...
        const Expression *operand1 = static_cast<const Expression *>(
            rhsParamList->getParamList()[0]->getExpr());
        const Expression *operand2 = static_cast<const Expression *>(
            rhsParamList->getParamList()[1]->getExpr());

        // Handle Operand1
        if (operand1->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
          const Memberaccess *operand1MemAccess =
              static_cast<const Memberaccess *>(operand1->getExpression());
          const Identifier *id1 = static_cast<const Identifier *>(
              operand1MemAccess->getIdentifier());
          const Identifier *id2 = static_cast<const Identifier *>(
              operand1MemAccess->getIdentifier2());
          ...
          if (id1 && id2) {
            ...
            if (isa<mlir::starplat::NodeType>(id1Op.getType())) {
              ...
              gOperand2 = mlir::starplat::GetNodePropertyOp::create(
                  builder, builder.getUnknownLoc(), builder.getI32Type(), id1Op,
        ...
        if (operand2->getKind() == ExpressionKind::KIND_ADDOP) {
          ...
          const Expression *addop1 =
              static_cast<const Expression *>(add->getOperand1());
          const Expression *addop2 =
              static_cast<const Expression *>(add->getOperand2());
          ...
          if (addop1->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
            const Memberaccess *op1MemberAccess =
                static_cast<const Memberaccess *>(addop1->getExpression());
            const Identifier *op1Id1 = static_cast<const Identifier *>(
                op1MemberAccess->getIdentifier());
            const Identifier *op1Id2 = static_cast<const Identifier *>(
                op1MemberAccess->getIdentifier2());

            if (globalLookupOp(op1Id1->getname()) &&
                globalLookupOp(op1Id2->getname())) {
              ...
              if (isa<mlir::starplat::NodeType>(op1id1op.getType())) {
                ...
                auto getProp = builder.getStringAttr(
                    op1Id2->getname()); // TODO: Add check here
                op1 = mlir::starplat::GetNodePropertyOp::create(
                    builder, builder.getUnknownLoc(), builder.getI32Type(),
                    op1id1op, op1id2op, getProp);
              }
          ...
          if (addop2->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
            const Memberaccess *op2MemberAccess =
                static_cast<const Memberaccess *>(addop2->getExpression());
            const Identifier *op2Id1 = static_cast<const Identifier *>(
                op2MemberAccess->getIdentifier());
            const Identifier *op2Id2 = static_cast<const Identifier *>(
                op2MemberAccess->getIdentifier2());

            if (globalLookupOp(op2Id1->getname()) &&
                globalLookupOp(op2Id2->getname())) {
              ...
              if (isa<mlir::starplat::EdgeType>(op2id1op.getType())) {
                // Generate getNodeProp
                auto getProp = builder.getStringAttr(
                    op2Id2->getname()); // TODO: Add check here
                op2 = mlir::starplat::GetEdgePropertyOp::create(
                    builder, builder.getUnknownLoc(), builder.getI32Type(),
                    op2id1op, op2id2op, getProp);
              }
              ...
              llvm::outs() << "Error: Undefined variables " << op2Id1->getname()
                           << " or " << op2Id2->getname() << "\n";
              exit(0);
            }
          } else {
          ...
          // Create add OP
          gOperand3 = mlir::starplat::AddOp::create(
              builder, builder.getUnknownLoc(), builder.getI32Type(),
      ...
      if (rhsexpr2->getKind() == ExpressionKind::KIND_KEYWORD) {
        const Keyword *rhsKeyword =
            static_cast<const Keyword *>(rhsexpr2->getExpression());
        if (globalLookupOp(rhsKeyword->getKeyword())) {
          gOperand4 = globalLookupOp(rhsKeyword->getKeyword());
        } else {
          llvm::outs()
              << "Error: Add this to Symbol Table and Pass the operation.\n";
          exit(0);
        }
      }
      ...
      // auto minOp =
      mlir::starplat::MinOp::create(
          builder, builder.getUnknownLoc(), builder.getI32Type(),
          gOperand2->getResult(0), gOperand1->getResult(0),
  ...
  virtual void visitAdd(const Add *add,
                        mlir::SymbolTable *symbolTable) override {}
  ...
  virtual void visitFunction(const Function *function,
    ...
    for (auto arg : args) {
      if (arg->getType() != nullptr) {
        ...
        if (std::string(arg->getTemplateType()
                            ->getGraphPropNode()
                            ->getPropertyType()) == "propNode") {
          argTypes.push_back(builder.getType<mlir::starplat::PropNodeType>(
              builder.getI32Type(), graphId));
          auto type = builder.getType<mlir::starplat::PropNodeType>(
              builder.getI32Type(), graphId);
          // auto typeAttr =
    ...
    auto func = mlir::starplat::FuncOp::create(
        builder, builder.getUnknownLoc(), function->getfuncNameIdentifier(),
  ...
  virtual void visitParamlist(const Paramlist *paramlist,
                              mlir::SymbolTable *symbolTable) override {
    std::vector<Param *> paramListVecvtor = paramlist->getParamList();
  ...
  virtual void visitFixedpointUntil(const FixedpointUntil *fixedpointuntil,
                                    mlir::SymbolTable *symbolTable) override {
    // Create new region.
  ...
  virtual void visitInitialiseAssignmentStmt(
    ...
    mlir::Value graph = NULL;
    auto idDecl = mlir::starplat::DeclareOp::create(
  ...
  virtual void visitMemberAccessAssignment(
    ...
    if (isa<mlir::starplat::NodeType>(type)) {
      ...
      if (expr->getKind() == ExpressionKind::KIND_NUMBER) {
        const Number *number =
            static_cast<const Number *>(expr->getExpression());
        ...
        mlir::starplat::SetNodePropertyOp::create(
            builder, builder.getUnknownLoc(),
            id2.getDefiningOp()->getOperand(0), id1, id2, keywordVal);
      }
    }
  }

  virtual void visitKeyword(const Keyword *keyword,
                            mlir::SymbolTable *symbolTable) override {
  ...
  virtual void visitGraphProperties(const GraphProperties *graphproperties,
                                    mlir::SymbolTable *symbolTable) override {}

  virtual void visitMethodcall(const Methodcall *methodcall,
                               mlir::SymbolTable *symbolTable) override {
    const Paramlist *paramlist =
  ...
  virtual void visitMemberaccess(const Memberaccess *memberaccess,
                                 mlir::SymbolTable *symbolTable) override {
    ...
    if (memberaccess->getIdentifier2()) {
      if (!globalLookupOp(builder.getStringAttr(
              memberaccess->getIdentifier2()->getname()))) {
        if (globalLookupOp(memberaccess->getIdentifier()->getname()) ==
            nullptr) {
          llvm::outs() << "Error2: "
                       << memberaccess->getIdentifier2()->getname()
                       << " not defined!\n";
  ...
  virtual void visitArglist(const Arglist *arglist,
                            mlir::SymbolTable *symbolTable) override {}

  virtual void visitArg(const Arg *arg,
                        mlir::SymbolTable *symbolTable) override {}

  virtual void visitStatement(const Statement *statement,
                              mlir::SymbolTable *symbolTable) override {}

  virtual void visitStatementlist(const Statementlist *stmtlist,
                                  mlir::SymbolTable *symbolTable) override {
  ...
  virtual void visitType(const TypeExpr *type,
                         mlir::SymbolTable *symbolTable) override {}
  ...
  virtual void visitExpression(const Expression *expr,
                               mlir::SymbolTable *symbolTable) override {
